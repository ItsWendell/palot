...
function CommandPaletteInput({
  ...
  return (
    <div data-slot="command-palette-input" className="flex h-14 items-center gap-3 px-4">
      <SearchIcon className="size-4 shrink-0 text-muted-foreground" aria-hidden="true" />
      <CommandPrimitive.Input
        data-slot="command-input"
        className={cn(
          "min-w-0 flex-1 bg-transparent text-sm outline-hidden placeholder:text-muted-foreground/70 disabled:cursor-not-allowed disabled:opacity-50",
          className,
        )}
        {...props}
      />
      {shortcut ? <Kbd className="shrink-0 bg-foreground/6">{shortcut}</Kbd> : null}
    </div>
  );
}
