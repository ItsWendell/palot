...
describe("OpenCode service compatibility", () => {
  ...
  it("accepts stable V2 additions without pretending every release was tested", () => {
    for (const version of ["2.0.10", "2.1.0", "2.99.123"]) {
  ...
  it("offers explicit consent for untested V2 betas, not incompatible stable builds", () => {
    expect(canContinueOpenCodeVersionMismatch("2.0.1")).toBe(false);
    expect(canContinueOpenCodeVersionMismatch("2.0.6")).toBe(false);
  ...
  it("reuses compatible stable services without requiring consent", () => {
    expect(shouldReuseOpenCodeService("2.0.7")).toBe(true);
    expect(shouldReuseOpenCodeService("2.0.6")).toBe(false);
    expect(shouldReuseOpenCodeService("0.0.0-beta-19507")).toBe(false);
