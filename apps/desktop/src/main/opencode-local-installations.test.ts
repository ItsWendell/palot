...
function harness(
  ...
  const versions: Record<string, string> = options.versions ?? { "/usr/bin/opencode": "2.0.7" };
  const aliases: Record<string, string> = options.aliases ?? {};
  const release: OpenCodeReleaseStatus = {
    channel: "stable",
    bundledVersion: "2.0.7",
...
describe("local OpenCode installations", () => {
  it("revalidates login startup consent and retains the package-manager launcher", async () => {
    ...
    h.versions["/usr/bin/opencode"] = "2.0.8";
    await expect(h.manager.loginBinary(installation.id, "2.0.7")).rejects.toThrow("changed");
    await expect(h.manager.loginBinary(installation.id, "2.0.8")).rejects.toThrow("changed");
    await expect(h.manager.loginBinary("/arbitrary/executable", "2.0.7")).rejects.toThrow(
      "no longer available",
    );
  });
  ...
  it("finds an exact installed SSH runtime without changing the user's local selection", async () => {
    const h = harness({
      versions: { "/new/opencode": "2.1.0", "/matching/opencode": "2.0.7" },
    });
    expect(await h.manager.discoverPreferredBinary({ exactVersion: "2.0.7" })).toEqual({
    ...
    expect(h.saved()).toBeUndefined();
    expect(await h.manager.discoverPreferredBinary({ exactVersion: "2.0.10" })).toBeNull();
    h.manager.setPreference("palot");
    expect(await h.manager.discoverPreferredBinary({ exactVersion: "2.0.7" })).toBeNull();
  });

  it("does not bypass explicit installed selection or OPENCODE_BIN for an exact SSH request", async () => {
    const h = harness({
      override: "/custom/opencode",
      versions: { "/custom/opencode": "2.1.0", "/matching/opencode": "2.0.7" },
    });
    expect(await h.manager.discoverPreferredBinary({ exactVersion: "2.0.7" })).toEqual({
    ...
    expect(await selected.manager.discoverPreferredBinary({ exactVersion: "2.0.7" })).toEqual({
      path: "/custom/opencode",
      version: "2.1.0",
    });
  });
  ...
  it("deduplicates canonical aliases, excludes app-owned files, and recognizes V2 compatibility", async () => {
    const h = harness({
      versions: {
        "/usr/bin/opencode": "2.0.7",
        "/old/opencode": "1.4.0",
        "/beta/opencode": "0.0.0-beta-19507",
        "/new-beta/opencode": "0.0.0-beta-99999",
        "/palot/data/opencode": "2.0.8",
        "/palot/resources/opencode": "2.0.8",
      },
      aliases: {
  ...
  it("preserves explicit override priority and refuses invalid overrides without falling back", async () => {
    const h = harness({
      override: "/custom/opencode",
      versions: { "/usr/bin/opencode": "2.0.7", "/custom/opencode": "2.0.10" },
    });
  ...
  it("prefers selection, persists it, and refuses missing or changed selected installations", async () => {
    const h = harness({
      versions: { "/usr/bin/opencode": "2.0.7", "/home/alice/.bun/bin/opencode": "2.0.10" },
    });
    ...
    h.versions["/home/alice/.bun/bin/opencode"] = "2.0.8";
  ...
  it("deduplicates concurrent inspection and bounds candidates and process concurrency", async () => {
    const h = harness({
      versions: Object.fromEntries(
        Array.from({ length: 50 }, (_, index) => [`/bin-${index}/opencode`, "2.0.7"]),
      ),
    });
    let active = 0;
    let maximum = 0;
    h.deps.run = vi.fn(async () => {
      ...
      active--;
      return { code: 0, stdout: "2.0.7", stderr: "" };
    });
  ...
  it.each<OpenCodeInstallMethod>(["auto", "npm", "bun", "pnpm", "yarn", "curl"])(
    ...
    async (method) => {
      ...
      expect(result.installations[0]!.version).toBe("2.0.8");
      expect(vi.mocked(h.deps.run).mock.calls.map(([, args]) => args)).toEqual([
        ["--version"],
        ["--version"],
        ["upgrade", "2.0.8", ...(method === "auto" ? [] : ["--method", method])],
        ["--version"],
      ]);
  ...
  it("rejects V1 existing runtimes, unsupported methods, and stale offers", async () => {
    ...
    await expect(
      old.manager.upgrade({ id: oldID, currentVersion: "1.0.0", version: "2.0.8", method: "auto" }),
    ...
    await expect(
      h.manager.upgrade({ id, currentVersion: "2.0.7", version: "2.0.9", method: "auto" }),
    ).rejects.toThrow("release changed");
    h.release.channel = "beta";
    await expect(
      h.manager.upgrade({ id, currentVersion: "2.0.7", version: "2.0.8", method: "auto" }),
    ).rejects.toThrow("release changed");
    expect(h.deps.run).toHaveBeenCalledTimes(1);
  });

  it("can upgrade an older stable CLI without accepting its incompatible service contract", async () => {
    const h = harness({ versions: { "/usr/bin/opencode": "2.0.3" } });
    const installed = (await h.manager.inspect()).installations[0]!;
    expect(installed.compatible).toBe(false);
    const result = await h.manager.upgrade({
      id: installed.id,
  ...
  it("requires exact-offer consent for Beta, including previously reviewed builds", async () => {
    const h = harness();
    const id = (await h.manager.inspect()).installations[0]!.id;
    h.release.channel = "beta";
  ...
  it("rejects an older dialog's installed-version consent after another inspection refreshes the same ID", async () => {
    ...
    expect(h.deps.run).not.toHaveBeenCalled();
    expect(h.versions[dialogInstallation.path]).toBe("2.0.8");
  });
  ...
  it.each(["version", "symlink", "offer"])(
    ...
    async (change) => {
      ...
      const id = (await h.manager.inspect()).installations[0]!.id;
      if (change === "version") h.versions["/usr/bin/opencode"] = "2.0.10";
  ...
  it("deduplicates updates and blocks selection/preference changes or competing updates", async () => {
    ...
    const input = { id, currentVersion: "2.0.7", version: "2.0.8", method: "auto" as const };
    ...
    await expect(h.manager.upgrade({ ...input, currentVersion: "2.0.10" })).rejects.toThrow(
      "in progress",
    );
  ...
  it("does not claim success when upstream updated the wrong installation", async () => {
    const h = harness();
    const id = (await h.manager.inspect()).installations[0]!.id;
    h.deps.run = vi.fn(async () => ({ code: 0, stdout: "2.0.7", stderr: "" }));
    await expect(
      h.manager.upgrade({ id, currentVersion: "2.0.7", version: "2.0.8", method: "auto" }),
    ).rejects.toThrow("Another installation");
    expect(h.manager.status().installations[0]!.version).toBe("2.0.7");
  ...
  it("never exposes subprocess output containing credentials on failures", async () => {
    ...
    const failure = await h.manager
      .upgrade({ id, currentVersion: "2.0.7", version: "2.0.8", method: "auto" })
  ...
  it("keeps approved installed Beta launchable with exact installation/version/SDK consent", async () => {
    const h = harness({ versions: { "/usr/bin/opencode": "2.0.7", "/other/opencode": "2.0.10" } });
    const result = await h.manager.inspect();
  ...
  it("preserves approved selection when an official upgrade retargets its symlink", async () => {
    const h = harness({
      ...
      versions: { "/package-v1/opencode": "2.0.7" },
    });
    const id = (await h.manager.inspect()).installations[0]!.id;
    h.manager.select(id);
    const run = h.deps.run;
    h.deps.run = vi.fn(async (...args: Parameters<OpenCodeInstallationDependencies["run"]>) => {
      ...
      h.versions["/package-v2/opencode"] = "2.0.8";
  ...
  it("does not let missing PATH entries exhaust the executable probe budget", async () => {
    ...
    expect((await h.manager.inspect()).installations[0]!.version).toBe("2.0.7");
    expect(h.deps.run).toHaveBeenCalledTimes(1);
  });
