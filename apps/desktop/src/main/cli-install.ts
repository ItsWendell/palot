 ...
 *
 * macOS packaged:  Nexus Builder.app/Contents/Resources/bin/nexus-builder
 * Linux packaged:  <install-dir>/resources/bin/nexus-builder
 * Windows:         <install-dir>/resources/bin/nexus-builder.cmd
 * Dev mode:        <repo>/apps/desktop/resources/bin/nexus-builder
 */
function getCliSourcePath(): string {
	const scriptName = process.platform === "win32" ? "nexus-builder.cmd" : "nexus-builder"
 ...
 * Windows:     %LOCALAPPDATA%\Nexus Builder\bin  (added to PATH by the user)
 */
function getCliInstallDir(): string {
	if (process.platform === "win32") {
		const localAppData =
			process.env.LOCALAPPDATA ?? path.join(app.getPath("home"), "AppData", "Local")
		return path.join(localAppData, "Nexus Builder", "bin")
	}
	return "/usr/local/bin"
}

/**
 * Returns the full path where the `nexus-builder` symlink/script will be placed.
 */
function getCliInstallPath(): string {
	const name = process.platform === "win32" ? "nexus-builder.cmd" : "nexus-builder"
...
/**
 * Installs the `nexus-builder` CLI command by symlinking it into /usr/local/bin
