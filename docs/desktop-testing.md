...
Builds a dev-only Palot window with disposable **palot** and **opencode** projects,
two slowly streaming conversations, two unread completed tasks, and a pending
question. The inbox opens alongside the active Palot conversation. You can switch
tasks and answer the question. Responses are scripted, not generated by a real
model; arbitrary follow-up prompts aren't supported.

The streams finish in about three minutes, then the window stays open until
Ctrl-C. Rerun the command for a fresh workspace. The runner uses the screenshot
tool's isolated OpenCode service and local test model, never your real projects,
sessions, or credentials. Nothing is added to the packaged app.

Interactive runs snapshot the most recently saved local Palot appearance,
including its Omarchy palette, into the temporary profile. Theme changes in the
demo don't affect your normal app. Set `PALOT_DEMO_APPEARANCE` to a specific
`appearance.json` to choose another profile. With no saved profile, the demo uses
system defaults. `--smoke` keeps the isolated defaults for reproducible checks.

This needs the same prerequisites as E2E, including the pinned OpenCode CLI and a
working display. Set `OPENCODE_BIN=/absolute/path/to/opencode` if the matching
binary isn't on PATH. It uses the normal cross-platform window, without the
macOS-only showcase capture path. Don't run it alongside a dev build in the same
checkout because both write to `apps/desktop/out`.

```sh
bun run demo --help
bun run demo --smoke        # Verify the staged workspace hidden, then clean up
bun run demo --smoke --keep # Also retain demo.png, demo.json, and run logs
```

Ctrl-C cleans up the owned Electron process, service, and temporary data. `--keep`
retains successful artifacts under `.local/desktop-e2e/`; failures are always
retained. `demo.json` records the staged session IDs for inspection.
