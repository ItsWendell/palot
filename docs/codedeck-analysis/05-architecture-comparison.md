...
**Codedeck (Browser mode):**

```
React App → HTTP (Hono RPC) → Codedeck Server (port 3100)
React App → OpenCode SDK (HTTP/SSE) → opencode serve (port 4101)
Codedeck Server → manages → opencode serve process
```
...
**Codedeck** runs two processes in Electron mode:
...
- Codedeck Hono server (port 3100) — discovery, model state, file system proxy
...
**Trade-offs:**
| | Codedeck | OpenCode TUI | OpenCode Desktop |
|---|---|---|---|
...
- Codedeck's explicit streaming store is more complex but gives better control
- SolidJS's fine-grained reactivity avoids the React 19 footguns Codedeck documents
...
**Codedeck's structure is simpler** (3 packages vs 7+), which makes it easier to navigate and contribute to. However, OpenCode's richer package structure reflects its much larger feature set.
...
**Codedeck's discovery is more complex** because it reads OpenCode's file storage directly (for offline access), then overlays live session data from the server. This means it can show session history even before the OpenCode server starts.
...
**Codedeck is missing pagination and virtual scrolling**, which will cause performance issues with long sessions (200+ messages, as indicated by the 200-message cap in `upsertMessage`).
...
- `window.codedeck` bridge exposes a limited API
- No authentication between Codedeck and OpenCode server (localhost only)
...
**OpenCode Desktop has a stronger security model** due to Tauri's Rust-based sandboxing and explicit server authentication. Codedeck's Electron-based approach is adequate for a desktop app but doesn't implement server authentication.
...
**Codedeck's Electron builds are significantly larger** (~100MB+ vs ~10MB for Tauri). This is the largest architectural disadvantage. The DESIGN.md mentions Tauri as the target, suggesting a future migration from Electron.
