...
Palot is a smaller, focused monorepo:

```
apps/
  desktop/    # @palot/desktop — Vite + React 19 app
  server/     # @palot/server — Bun + Hono backend
packages/
  ui/         # @palot/ui — Shared shadcn/ui components
```
...
SolidJS's fine-grained reactivity means OpenCode only re-renders the exact DOM nodes that depend on changed data. React re-renders entire component subtrees, requiring careful memoization. Palot mitigates this with binary search + structural sharing, but it's fundamentally more work.

### Server Architecture

| Aspect | OpenCode | Palot |
|--------|----------|----------|
| Server | Single Hono server (port 4096) | Two servers (Palot 3100 + OpenCode 4101) |
...
Palot's two-server architecture adds complexity. The Palot backend server exists primarily to:
...
OpenCode's event bus is bidirectional — internal systems both publish and subscribe. Palot is a passive consumer of OpenCode's SSE stream, handling only the events needed for the UI.
...
Palot's lack of persistent preferences means UI state (sidebar position, theme, display preferences) resets on every page reload. OpenCode persists 15+ UI preferences to its KV store.
