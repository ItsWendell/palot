# Recommended Improvements for Palot

Prioritized improvements Palot should implement, organized by impact and effort. Each item references the OpenCode feature it's inspired by and why it matters for a GUI dashboard.

## Priority Tiers

- **P0 (Critical)** — Missing features that significantly hurt usability
- **P1 (High)** — Features that would meaningfully differentiate Palot
...
**Why it matters:** Without cost visibility, users have no idea how much they're spending. This is table stakes for any AI tool dashboard. The `tokenlens` library is already in Palot's dependencies but unused.
...
**Why it matters:** Silent failures are the worst UX. When something goes wrong, users need clear feedback. Palot currently swallows most errors.
...
**What Palot needs:** Expand the existing `Cmd+K` palette from just sessions to:
...
**What Palot needs (in priority order):**
...
Adopt Agent Client Protocol for standardized multi-agent communication, enabling Palot to orchestrate agents beyond just OpenCode.

**Effort:** VERY HIGH

### 21. PTY / Terminal Integration

Embed a terminal in Palot for direct shell access alongside agent sessions.

**Effort:** HIGH (Tauri prerequisite)

### 22. Inline Code Editor

Allow users to edit files directly in Palot with syntax highlighting, rather than switching to their editor.
