# Palot Strengths: Where Palot Excels

> Areas where Palot is better than, different from, or has unique advantages over OpenCode TUI and OpenCode Desktop.

## 1. Streaming Performance Architecture

**Palot's dual-store streaming approach is more sophisticated than OpenCode Desktop's.**

Palot separates streaming state from main state using two stores:
...
**Verdict:** Palot's explicit streaming architecture gives it better control over performance during heavy streaming, especially on lower-powered machines. This is a genuine engineering advantage.

## 2. Multi-Project Dashboard Experience

**Palot is designed as a multi-project management tool from day one.**

Palot's sidebar organizes sessions into three sections:
...
**Verdict:** Palot's flat "Active Now" + "Recent" sections across all projects is a better UX for users managing multiple concurrent agents. You see everything that matters without clicking into project folders.

## 3. Sub-Agent Visibility Toggle

**Palot has a dedicated toggle for showing/hiding sub-agent sessions.**
...
**Palot offers three display density modes: Default, Compact, and Verbose.**
...
**Palot color-codes tool calls by category with left-border accents.**
...
**Palot's `packages/ui` has an extensive `ai-elements/` directory with 50+ components.**
...
**Palot shows user messages and file attachments immediately before server confirmation.**
...
**OpenCode TUI** also does optimistic updates via the SolidJS store in `sync.tsx`, but Palot's implementation is particularly clean with the binary-search-based replacement logic.

**OpenCode Desktop** uses `applyOptimisticAdd` in its sync context for the same purpose.

**Verdict:** Parity, but Palot's implementation is well-documented and clean.

## 8. Draft Persistence

**Palot persists prompt drafts across session switches and page reloads.**
...
**Verdict:** Palot's automatic draft persistence is a small but meaningful UX improvement. Users don't lose their half-typed prompts when switching sessions.

## 9. Attach-from-Terminal Feature

**Palot provides "Attach from terminal" functionality in both the sidebar and per-session.**
...
This creates a bridge between Palot's GUI and OpenCode's TUI, allowing users to:

1. Start a session in Palot
...
**Palot's migration to a single OpenCode server is architecturally simpler than multi-server.**
...
**Palot's DESIGN.md outlines capabilities neither OpenCode version has:**
