# UI Improvements for Palot

> Specific visual and component-level improvements Palot should make, informed by OpenCode TUI and Desktop patterns.
...
**Current state:** `<MessageResponse>` component from `@palot/ui` renders text. It's unclear if full markdown is supported.
...
- The `@palot/ui` package has `@streamdown` in its dependencies, suggesting this may be partially implemented
