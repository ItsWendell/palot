# Feature Gaps: Palot vs OpenCode TUI & Desktop

> Features present in OpenCode TUI and/or OpenCode Desktop that Palot currently lacks.

## Critical Gaps (Blocking Core Workflows)

### 1. Session Operations

| Feature                            |            OpenCode TUI             |       OpenCode Desktop        |             Palot             |
| ---------------------------------- | :---------------------------------: | :---------------------------: | :------------------------------: |
...
**Impact:** Palot's permission system shows tool name + command but NOT the actual diff content. Users approve file edits blind. This is a major trust/safety issue. The session review panel with file diffs is one of OpenCode Desktop's strongest features that Palot completely lacks.
...
**Impact:** Users cannot browse project files or review code changes. They must use an external editor alongside Palot, losing the "unified workspace" value prop.
...
**Impact:** The prompt input in Palot is a plain textarea with file attachment support. No autocomplete, no command system, no file mentions. This is one of the highest-impact gaps because it directly affects every interaction.
...
**Impact:** Users cannot connect new providers or manage MCP servers from within Palot. They must configure everything externally via the OpenCode CLI first.
