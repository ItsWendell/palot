...
- **Built registry**: generated JSON files served to CLI consumers, usually
  from `public/r`. Use `vp exec -- shadcn build` to create this form.
...
```bash
vp exec -- shadcn build
vp exec -- shadcn build registry.json --output public/r
```

Use CLI commands to inspect the result:

```bash
vp exec -- shadcn list @acme
vp exec -- shadcn search @acme -q "login"
vp exec -- shadcn view @acme/login-form
vp exec -- shadcn add @acme/login-form --dry-run
vp exec -- shadcn registry validate ./registry.json
```

Use GitHub addresses directly for public GitHub registries:

```bash
vp exec -- shadcn list owner/repo
vp exec -- shadcn search owner/repo -q "login"
vp exec -- shadcn view owner/repo/item
vp exec -- shadcn add owner/repo/item --dry-run
vp exec -- shadcn registry validate owner/repo
```
